...
class Configuration:
    ...
    def __init__(self):
        ...
        self.credential = ChainedTokenCredential(
                ...
                AzureCliCredential(tenant_id=self.tenant_id)
            )
        
        self.aiocredential = AsyncChainedTokenCredential(
                AsyncManagedIdentityCredential(client_id=self.client_id),
                AsyncAzureCliCredential(tenant_id=self.tenant_id)
            )
        
        labels = self.tenant_id = os.environ.get('LABELS', ["*", "gpt-rag"])

        self._init_clients(labels=labels)

    @retry(
        stop=stop_after_attempt(5)
    )
    def _init_clients(self, labels = ["*", "gpt-rag"]):

        selectors = []
        
        for label in labels:
            selectors.append(SettingSelector(label_filter=label, key_filter='*'))
        ...
        try:
            self.config = load(selects=selectors, endpoint=app_config_uri, credential=self.credential, key_vault_options=AzureAppConfigurationKeyVaultOptions(credential=self.credential))
        except Exception as e:
            logging.info(f"Unable to connect to Azure App Configuration. Please check app config url setting. {e}")
            try:
                connection_string = os.environ["AZURE_APPCONFIG_CONNECTION_STRING"]
                # Connect to Azure App Configuration using a connection string.
                self.config = load(selects=selectors,connection_string=connection_string, key_vault_options=AzureAppConfigurationKeyVaultOptions(credential=self.credential))
            except Exception as e:
