...
class ContentExtractorStep(StepBase):
    ...
    async def run(self, input_data: StepInputOutput, context: "PipelineExecutionContext") -> Dict:
        """
        ...
        """
        document = await self.get_document(input_data.id, context)
        await self.process_document(document, context)
        input_data.data['document'] = document
