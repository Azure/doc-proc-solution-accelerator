...
class BlobService(ServiceBase):
    ...
    def __init__(self, name: str, type: str, settings:dict, **kwargs):
        ...
        self.storage_account_name = self.settings.get('account_name')
        ...
        if self.credential_type == 'azure_key_credential':
            self.credential_key = self.settings.get('credential_key')
    ...
    def get_file_metadata(self, container_name:str, filename:str):

        container_client = self.get_container_client(container_name)

        file_blob_client = container_client.get_blob_client(filename)

        file_properties = file_blob_client.get_blob_properties()
        return file_properties

    def get_file(self, container_name:str, filename:str):

        container_client = self.get_container_client(container_name)

        file_blob_client = container_client.get_blob_client(filename)

        file_content = file_blob_client.download_blob()
        return file_content.readall()

    def upload_file(self, container_name:str, filename:str, file_content):
        
        container_client = self.get_container_client(container_name)

        file_blob_client = container_client.upload_blob(name=filename, data=file_content, overwrite=False)

        return file_blob_client.url
    
class AsyncBlobService(ServiceBase):
    """Azure Blob Storage service for managing blob storage operations."""
    ...
    async def _connect(self):
        ...
        if self.credential_key not in ['', None]:
            self.blob_service_client = AsyncBlobServiceClient(account_url=account_url, credential=self.credential_key)
        else:
            self._get_credentials()
            self.blob_service_client = AsyncBlobServiceClient(account_url=account_url, credential=self.aiocredential)
